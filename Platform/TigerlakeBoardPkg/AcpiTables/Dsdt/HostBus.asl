...
//    ReadWrite,0x00,0xF00000,0xFFFFFF,0x00,0x100000,,,HOLE)
  ...
  //
  // PCI Memory Region ( TOLUD - ACPI_MMIO_BASE_ADDRESS )
  //
  ...
  // a new region after PCI_EXPRESS_BASE + 256M: 0xD0000000 - 0xDFFFFFFF
  DWordMemory(ResourceProducer,PosDecode,MinFixed,MaxFixed,NonCacheable,
    ReadWrite,0x00000000,0xD0000000,0xDFFFFFFF,0x00000000,0x10000000,,,PM03)
...
Method(GEPB,0,Serialized)
{
  if(LEqual(EP_B,0))
  {
    ShiftLeft(\_SB.PC00.EPBR,12,EP_B)
  }
  Return(EP_B)
}
...
Method(GMHB,0,Serialized)
{
  if(LEqual(MH_B,0))
  {
    ShiftLeft(\_SB.PC00.MHBR,15,MH_B)
  }
  Return(MH_B)
}
...
{
  if(LEqual(PC_L,0)) {
    if(LLess (\_SB.PC00.PXSZ, 3)) {
      ShiftRight(0x10000000, \_SB.PC00.PXSZ,PC_L)
    } Else {
...
Method(GDMB,0,Serialized)
{
  if(LEqual(DM_B,0))
  {
    ShiftLeft(\_SB.PC00.DIBR,12,DM_B)
  }
  Return(DM_B)
}
