...
/**
  ...
  update driver. Since HECI functionality is available to access
  only in silicon driver, this init is done in silicon package.

  @retval Pointer to initialized input data structure.
   ...
   )
{
  ...
  //FlashDescMasterAccess[] = {R_FLMSTR1_BIOS, R_FLMSTR2_ME, R_FLMSTR3_GBE, R_FLMSTR4_RSVD, R_FLMSTR5_EC};
  UINT32 FlashDescMasterAccess[] = {0x0, 0x0, 0x0, 0x0, 0x0};

  //Get SPI Descriptor mode status
  ...
  if (SpiProtectionMode == 0) {
    ...
    DEBUG((DEBUG_INFO, "FlashDescMasterAccess[0] 0x%x\n", FlashDescMasterAccess[0]));
    DEBUG((DEBUG_INFO, "FlashDescMasterAccess[1] 0x%x\n", FlashDescMasterAccess[1]));
    DEBUG((DEBUG_INFO, "FlashDescMasterAccess[2] 0x%x\n", FlashDescMasterAccess[2]));
    DEBUG((DEBUG_INFO, "FlashDescMasterAccess[3] 0x%x\n", FlashDescMasterAccess[3]));
    DEBUG((DEBUG_INFO, "FlashDescMasterAccess[4] 0x%x\n", FlashDescMasterAccess[4]));

    // Flash descriptor region read/write access for flash master CPU/BIOS
    FlashDescMasterAccess[0] |= MASTER_ACCESS_BIOS_SET_REGION;
    FlashDescMasterAccess[0] &= ~MASTER_ACCESS_BIOS_CLEAR_REGION;

    // Flash descriptor region read/write access for flash master ME
    FlashDescMasterAccess[1] |= MASTER_ACCESS_ME_SET_REGION ;
    FlashDescMasterAccess[1] &= ~MASTER_ACCESS_ME_CLEAR_REGION;

    // Flash descriptor region read/write access for flash master Gbe
    FlashDescMasterAccess[2] |= MASTER_ACCESS_GBE_SET_REGION;
    FlashDescMasterAccess[2] &= ~MASTER_ACCESS_GBE_CLEAR_REGION;

    // Flash descriptor region read/write access for flash master EC
    FlashDescMasterAccess[4] |= MASTER_ACCESS_EC_SET_REGION;
    FlashDescMasterAccess[4] &= ~MASTER_ACCESS_EC_CLEAR_REGION;
    ...
    DEBUG((DEBUG_INFO, "SPI is in Protected Mode. Operation is not performed!!"));
